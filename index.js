...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\ seja bem vindo/a ao grupo *${mdata.subject}*!\nAguarde as orientações dos ADMs, logo eles irão te explicar tdo.`
				...
				teks = `Tchau @${num.split('@')[0]} !!\nSe quiser voltar para o pj... volte para o recrutamento.`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					Badmin: '❬❗❭ O bot precisa ser ADM ☺️'
				}
			}
			...
			case 'loli': 
               ...
               if (!isGroup) return reply(mess.only.group)
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('Não tenho bola de cristal pra adivinhar quem vc quer add Aki rsrsrs')
					...
					try {
						...
						console.log('Error :', e)
						reply('Falha ao adicionar, talvez porque o whats seja privado...')
					}
					break
					case 'grup':
					case 'group':
					...
					if (args[0] === 'buka') {
						...
						reply(`*O GRUPO FOI FECHADO PARA ADMs 🔒.*`)
           ...
           case 'demote':
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `Rebaixado à membro:\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					} else {
						mentions(`@${mentioned[0].split('@')[0]} perdeu o cargo de ADM´, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'promote':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Marca a pessoa pra tornar ADM.')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
						...
						mentions(`Novo adm fml... @${mentioned[0].split('@')[0]} esse mereceu ser ADM.`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break	
			     	case 'kick':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Removi mermo @${mentioned[0].split('@')[0]} 🏃`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
					...
					break
				case 'toimg':
					if (!isQuotedSticker) return reply('Marca o sticker inteligente.')
					reply(mess.wait)
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('Algo deu erro... tente dnv dps ^_^')
				...
				case 'simih':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('Escolha 0 para desativar e 1 para ativar.')
					if (Number(args[0]) === 1) {
